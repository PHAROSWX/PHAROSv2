...
def load_thread():
    ...
    try:
        ...
        # ── GEFS: runs only at 00/06/12/18z — probe independently ───────────
        prog(60, 'Finding latest NOAA GEFS run...')
        gefs_data = None
        for delta_h in range(0, 48, 6):
            dt        = now_utc - timedelta(hours=delta_h)
            gh        = (dt.hour // 6) * 6
            gefs_cand = dt.replace(hour=gh, minute=0, second=0, microsecond=0)
            gdate     = gefs_cand.strftime('%Y%m%d')
            grun      = '%02d' % gefs_cand.hour
            probe_url = ('https://noaa-gefs-pds.s3.amazonaws.com/gefs.%s/%s/atmos/pgrb2ap5/'
                         'gec00.t%sz.pgrb2a.0p50.f000' % (gdate, grun, grun))
            try:
                rp = requests.head(probe_url, timeout=8)
                if rp.status_code == 200:
                    log.info('GEFS run: %s/%sz (all 31 members)', gdate, grun)
                    ens = GEFSEnsemble(tmpdir, flat_la.tolist(), flat_lo.tolist(),
                                       (gdate, grun))
                    gefs_data = ens.run()
                    log.info('GEFS data built: %d steps', len(gefs_data) if gefs_data else 0)
                    break
                else:
        ...
        if not icon_data and not gefs_data:
            prog(0, 'No model data available — ICON and GEFS both failed')
            return

        # Build times list from ICON or GEFS step count
        n_steps = len(icon_data) if icon_data else len(gefs_data)
