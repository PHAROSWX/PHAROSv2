...
def fetch_all(tmpdir):
    ...
    # Use AWS S3 mirror — avoids 429 rate limits on the direct ECMWF portal.
    # Falls back through Azure and Google Cloud if AWS also fails.
    # Try cloud mirrors first to avoid portal rate limits.
    # Set small number of retries so we don't wait forever on SlowDown/429.
    client = None
    for _src in ['aws', 'azure', 'google', 'ecmwf']:
        try:
            client = Client(source=_src, model='ifs', resol='0p25',
                            maximum_retries=3, retry_after=10)
            log.info('ECMWF: using source=%s', _src)
            break
        except TypeError:
            # Older ecmwf-opendata doesn't support maximum_retries
            try:
                client = Client(source=_src, model='ifs', resol='0p25')
                log.info('ECMWF: using source=%s (no retry cap)', _src)
                break
            except Exception as _e:
                log.debug('ECMWF source %s unavailable: %s', _src, _e)
        except Exception as _e:
            log.debug('ECMWF source %s unavailable: %s', _src, _e)
    if client is None:
        raise RuntimeError('No ECMWF source available')
    ...
    for pname, label in [('mucape', 'MUCAPE')]:
        ...
        _time.sleep(3)  # rate limit: pause between requests
...
def _fetch_openmeteo_grid(model_name, ref_lats, ref_lons,
    ...
    # Regional models (ICON-D2, AROME) may not have 250hPa or upper level winds
    if reduced_vars:
        PL_VARS = []
        for lv in [925, 850, 700, 500]:
    ...
    for si, step in enumerate(STEPS):
        ...
        u250  = get_var('wind_u_component_250hPa', si) if not reduced_vars else get_var('wind_u_component_500hPa', si)
        v250  = get_var('wind_v_component_250hPa', si) if not reduced_vars else get_var('wind_v_component_500hPa', si)
...
def fetch_icon_d2(ref_lats, ref_lons):
    ...
    return _fetch_openmeteo_grid('icon_d2', ref_lats, ref_lons,
        sample_step=0.5, max_workers=20, forecast_hours=48, label='ICON-D2',
        lat_min=43.0, lat_max=58.0, lon_min=1.0, lon_max=22.0,
        reduced_vars=True)


def fetch_ecmwf_openmeteo(ref_lats, ref_lons):
    """ECMWF IFS 0.25° via Open-Meteo — free, no auth, no rate limits."""
    log.info('ECMWF: fetching via Open-Meteo (ecmwf_ifs)...')
    return _fetch_openmeteo_grid('ecmwf_ifs', ref_lats, ref_lons,
        sample_step=1.0, max_workers=20, forecast_hours=72, label='ECMWF')
...
def fetch_cmc(ref_lats, ref_lons):
    ...
    return _fetch_openmeteo_grid('gem_seamless', ref_lats, ref_lons,
        sample_step=1.0, max_workers=20, forecast_hours=72, label='CMC')
...
def fetch_arpege_openmeteo(ref_lats, ref_lons):
    ...
    return _fetch_openmeteo_grid('meteofrance_arpege_world', ref_lats, ref_lons,
        sample_step=1.0, max_workers=20, forecast_hours=72, label='ARPEGE')
...
def fetch_arome(ref_lats, ref_lons):
    ...
    return _fetch_openmeteo_grid('meteofrance_arome_france', ref_lats, ref_lons,
        sample_step=1.0, max_workers=20, forecast_hours=42, label='AROME',
...
def load_thread():
    ...
    try:
        ...
        # ── ECMWF IFS (native ecmwf-opendata, AWS S3 mirror, rate-limited) ──
        prog(55, 'Fetching ECMWF IFS...')
        ecmwf_data = None
        import concurrent.futures as _cf_ecmwf
        def _do_ecmwf():
            files, run = fetch_all(tmpdir)
            if not files:
                return None
            res = build_grid(files, run)
            if isinstance(res, dict):
                return res.get('data')
            return res if isinstance(res, list) else None
        with _cf_ecmwf.ThreadPoolExecutor(max_workers=1) as _ecmwf_ex:
            _ecmwf_fut = _ecmwf_ex.submit(_do_ecmwf)
            try:
                ecmwf_data = _ecmwf_fut.result(timeout=1800)  # 30 min hard cap
                log.info('ECMWF data built: %d steps',
                         len(ecmwf_data) if ecmwf_data else 0)
            except _cf_ecmwf.TimeoutError:
                log.warning('ECMWF: timed out after 30 min — skipping')
                _ecmwf_fut.cancel()
            except Exception as e:
        ...
        _OM = {'cmc': fetch_cmc, 'arpege': fetch_arpege_openmeteo, 'arome': fetch_arome, 'icon_d2': fetch_icon_d2}
        ...
        icon_d2_data = _om['icon_d2']

        gefs_data = None  # GEFS removed

        if not any([icon_data, ecmwf_data, gfs_det_data, cmc_data, arpege_data, arome_data, icon_d2_data]):
            prog(0, 'No model data available — all sources failed')
            return

        # Build times list from ICON or GEFS step count
        n_steps = len(next(d for d in [icon_data,ecmwf_data,gfs_det_data,cmc_data,arpege_data,arome_data,icon_d2_data] if d))
