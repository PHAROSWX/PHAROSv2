...
# ═══════════════════════════════════════════════════════════════════════════════
# GFS DETERMINISTIC  (NOAA public S3, 0.25°, 4×/day)
# Uses .idx sidecar files for byte-range requests — only downloads the
# specific parameters needed (~3 MB/step instead of ~500 MB full GRIB2).
# ═══════════════════════════════════════════════════════════════════════════════

def fetch_gfs_det(tmpdir, ref_lats, ref_lons):
    """
    Fetch NOAA GFS 0.25° deterministic forecast.
    Returns list of step dicts in same format as build_icon_grid.
    """
    from scipy.spatial import cKDTree
    import cfgrib
    ...
    for delta_h in range(0, 25, 6):
        ...
        try:
            if requests.head(probe, timeout=8).status_code == 200:
                gfs_run = (ds, rs)
                log.info('GFS run: %s/%sz', ds, rs)
                break
        except Exception:
            pass

    if not gfs_run:
        log.warning('GFS: no available run found')
        return None

    date_s, run_s = gfs_run

    # ── 2. Parameters we want from the idx file ───────────────────────────────
    # Each entry: idx search string → output key (plus K→°C flag)
    WANT = [
        ('CAPE:surface',               'cape',  False),
        ('CIN:surface',                'cin',   False),
        ('TMP:925 mb',                 't925',  True),
        ('TMP:850 mb',                 't850',  True),
        ('TMP:700 mb',                 't700',  True),
        ('TMP:500 mb',                 't500',  True),
        ('RH:925 mb',                  'r925',  False),
        ('RH:850 mb',                  'r850',  False),
        ('RH:700 mb',                  'r700',  False),
        ('UGRD:925 mb',                'u925',  False),
        ('UGRD:850 mb',                'u850',  False),
        ('UGRD:700 mb',                'u700',  False),
        ('UGRD:500 mb',                'u500',  False),
        ('UGRD:250 mb',                'u250',  False),
        ('VGRD:925 mb',                'v925',  False),
        ('VGRD:850 mb',                'v850',  False),
        ('VGRD:700 mb',                'v700',  False),
        ('VGRD:500 mb',                'v500',  False),
        ('VGRD:250 mb',                'v250',  False),
        ('PWAT:entire atmosphere',     'pwat',  False),
    ]

    def parse_idx(idx_text, want_list, file_size):
        """Parse GFS .idx to get byte ranges for wanted parameters."""
        ...
        for i, line in enumerate(lines):
            ...
            if len(parts) < 6:
                continue
            _, offset_s, _, param, level, *_ = parts
            key = f'{param}:{level}'
            for want_key, out_key, k2c in want_list:
                if want_key == key:
                    start = int(offset_s)
                    end = int(lines[i+1].split(':')[1]) - 1 if i+1 < len(lines) else file_size - 1
                    ranges[out_key] = (start, end, k2c)
                    break
        return ranges
    ...
    n = len(flat_la)
    tree = idxs = far = None
    raw = {k: {} for _, k, _ in WANT}   # {key: {step: array}}

    # ── 3. Download each step ─────────────────────────────────────────────────
    for step in STEPS:
        ...
        try:
            # Get idx file
            ir = requests.get(idx_url, timeout=15)
            if not ir.ok:
                log.debug('GFS idx missing step %d: HTTP %d', step, ir.status_code)
                continue

            # Get file size from HEAD
            hr = requests.head(url, timeout=8)
            file_size = int(hr.headers.get('Content-Length', 999_999_999))

            ranges = parse_idx(ir.text, WANT, file_size)
            if not ranges:
                log.debug('GFS step %d: no wanted params found in idx', step)
                continue

            # Merge overlapping/adjacent ranges into minimal set of requests
            sorted_ranges = sorted(set((s, e) for s, e, _ in ranges.values()))
            merged = []
            for s, e in sorted_ranges:
                if merged and s <= merged[-1][1] + 50_000:
                    merged[-1] = (merged[-1][0], max(merged[-1][1], e))
                else:
                    merged.append([s, e])

            # Download merged byte ranges
            chunks = {}
            for s, e in merged:
                try:
                    rr = requests.get(url, headers={'Range': f'bytes={s}-{e}'}, timeout=30)
                    if rr.status_code in (200, 206):
                        chunks[(s, e)] = (rr.content, s)
                except Exception as de:
                    log.debug('GFS step %d range %d-%d: %s', step, s, e, de)

            if not chunks:
                continue

            # cfgrib requires real file paths — write each chunk to a temp file
            step_fields = {}
            chunk_tmp_dir = Path(tmpdir) / f'gfs_step{step:03d}'
            chunk_tmp_dir.mkdir(exist_ok=True)

            for ci, ((cs, ce), (data, offset)) in enumerate(chunks.items()):
                tmp_grib = chunk_tmp_dir / f'chunk_{ci}.grib2'
                try:
                    tmp_grib.write_bytes(data)
                    with suppress_eccodes_stderr():
                        dsets = cfgrib.open_datasets(
                            str(tmp_grib), indexpath=None
                        )
                    for ds in dsets:
                        lats = ds.latitude.values
                        ...
                        # Build KDTree once from first successful decode
                        if tree is None and fl_la.size > 100:
                            ...
                            if m_la.size == 0:
                                log.warning('GFS: no points in EU bounding box')
                                continue
                            _tree = cKDTree(np.column_stack([m_la, m_lo]))
                            dists, _idxs_raw = _tree.query(
                                np.column_stack([flat_la, flat_lo]), k=1)
                            idxs = np.where(mask)[0][_idxs_raw]
                            far  = dists > 1.0
                            log.debug('GFS KDTree built: %d source pts, %d target pts',
                                      m_la.size, flat_la.size)
                        ...
                        for var in ds.data_vars:
                            ...
                            for c in ('isobaricInhPa', 'pressure', 'level'):
                                if c in ds.coords:
                                    lev_coord = float(np.atleast_1d(
                                        ds.coords[c].values)[0])
                                    break
                            if arr.ndim > 2:
                                arr = arr.squeeze()
                            if arr.ndim != 2:
                            ...
                            if flat_v.size != fl_la.size:
                                continue

                            lv = int(round(lev_coord)) if lev_coord else 0
                            key_map = {
                                ('cape', 0):  'cape', ('cin', 0):  'cin',
                                ('pwat', 0):  'pwat',
                                ('t', 925):   't925', ('t', 850):   't850',
                                ('t', 700):   't700', ('t', 500):   't500',
                                ('r', 925):   'r925', ('r', 850):   'r850',
                                ('r', 700):   'r700',
                                ('u', 925):   'u925', ('u', 850):   'u850',
                                ('u', 700):   'u700', ('u', 500):   'u500',
                                ('u', 250):   'u250',
                                ('v', 925):   'v925', ('v', 850):   'v850',
                                ('v', 700):   'v700', ('v', 500):   'v500',
                                ('v', 250):   'v250',
                            }
                            out_key = key_map.get((var.lower(), lv))
                            if out_key:
                                if out_key.startswith('t') and out_key[1:].isdigit():
                                    if flat_v.mean() > 100:
                                        flat_v = flat_v - 273.15
                                step_fields[out_key] = flat_v
                except Exception as pe:
                    log.debug('GFS step %d chunk %d parse: %s', step, ci, pe)
                finally:
                    try: tmp_grib.unlink()
                    except Exception: pass

            try: chunk_tmp_dir.rmdir()
            except Exception: pass
            ...
            log.debug('GFS step %dh: %d fields', step, len(step_fields))

        except Exception as e:
            log.debug('GFS step %d: %s', step, e)

    if idxs is None or not any(raw.get('cape')):
        log.warning('GFS: no usable data parsed')
        return None

    # ── 4. Build step dicts (same format as build_icon_grid) ─────────────────
    ...
    for step in STEPS:
        ...
        r925 = get_g('r925', step, 75.0); r850 = get_g('r850', step, 60.0)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ARPÈGE (Météo-France open data, 0.1°, 4×/day)
# Free API key required — register at portail-api.meteofrance.fr
# Set env var METEOFRANCE_API_KEY or GitHub secret METEOFRANCE_API_KEY.
# Gracefully skips if key not available.
# ═══════════════════════════════════════════════════════════════════════════════

def fetch_arpege(tmpdir, ref_lats, ref_lons):
    """
    Fetch Météo-France Arpège 0.1° from their open data API.
    Skips silently if METEOFRANCE_API_KEY env var is not set.

    Key notes from MF docs:
    - The raw API key must be Base64-encoded before use in the header
    - Coverage IDs are discovered dynamically via GetCapabilities
    - URL path uses '01' not '010': MF-NWP-GLOBAL-ARPEGE-01-EUROPE-WCS
    """
    import tempfile
    from scipy.spatial import cKDTree
    import cfgrib

    api_key_raw = os.environ.get('METEOFRANCE_API_KEY', '')
    # Strip ALL whitespace including newlines — the token is a single string
    api_key_raw = ''.join(api_key_raw.split())
    if not api_key_raw:
        log.info('Arpège: METEOFRANCE_API_KEY not set — skipping')
        return None

    log.info('Arpège: token length=%d first8=%s', len(api_key_raw), api_key_raw[:8])

    # Météo-France (IBM API Connect) requires both the JWT Bearer token
    # AND the X-IBM-Client-Id header (the UUID from the token-generation URL).
    client_id = os.environ.get('METEOFRANCE_CLIENT_ID', '').strip()
    HEADERS = {
        'Authorization':   f'Bearer {api_key_raw}',
        'Accept':          'application/x-grib2',
    }
    if client_id:
        HEADERS['X-IBM-Client-Id'] = client_id
        log.info('Arpège: using Client-Id %s...', client_id[:8])
    else:
        log.warning('Arpège: METEOFRANCE_CLIENT_ID not set — trying without it')
    # MF API uses path-based routing — GetCapabilities is a URL path segment.
    # Try multiple known endpoint names and base URL patterns.
    BASE = None
    for base_prefix in [
        'https://public-api.meteofrance.fr/public/arpege/1.0',
        'https://public-api.meteofrance.fr/public/arpege',
    ]:
        for WCS_NAME in [
            'MF-NWP-GLOBAL-ARPEGE-005-EURAT-WCS',
            'MF-NWP-GLOBAL-ARPEGE-01-EUROPE-WCS',
            'MF-NWP-GLOBAL-ARPEGE-025-GLOBE-WCS',
        ]:
            test_url = f'{base_prefix}/wcs/{WCS_NAME}/GetCapabilities'
            try:
                caps_test = requests.get(test_url, headers=HEADERS, timeout=15)
                log.info('Arpège probe %s: HTTP %d — %s',
                         test_url, caps_test.status_code, caps_test.text[:120])
                if caps_test.ok:
                    BASE = f'{base_prefix}/wcs/{WCS_NAME}'
                    log.info('Arpège: using endpoint %s', WCS_NAME)
                    break
            except Exception as e:
                log.info('Arpège probe %s: %s', test_url, e)
        if BASE:
            break

    if not BASE:
        log.warning('Arpège: no WCS endpoint responded — see probe logs above')
        return None

    # ── 1. GetCapabilities to discover actual coverage IDs ────────────────────
    log.info('Arpège: fetching GetCapabilities...')
    try:
        caps_r = requests.get(f'{BASE}/GetCapabilities',
                              headers=HEADERS, timeout=30)
        if not caps_r.ok:
            log.warning('Arpège GetCapabilities failed: HTTP %d — %s',
                        caps_r.status_code, caps_r.text[:200])
            return None
        caps_text = caps_r.text
        log.debug('Arpège capabilities: %d chars', len(caps_text))
    except Exception as e:
        log.warning('Arpège GetCapabilities error: %s', e)
        return None

    # Parse coverage IDs from the XML
    import re as _re
    all_cov_ids = _re.findall(r'<wcs:Identifier>(.*?)</wcs:Identifier>', caps_text)
    if not all_cov_ids:
        # Try alternative XML format
        all_cov_ids = _re.findall(r'<ows:Identifier>(.*?)</ows:Identifier>', caps_text)
    log.info('Arpège: found %d coverage IDs in capabilities', len(all_cov_ids))
    if all_cov_ids:
        log.debug('Sample IDs: %s', all_cov_ids[:5])

    # Map our needed params to coverage ID substrings to search for
    WANT_MAP = {
        'cape':  ['CAPE', 'cape'],
        'cin':   ['CIN', 'cin'],
        't925':  ['TEMPERATURE__ISOBARIC', 'T__ISOBARIC', 'TEMP__ISOBARIC'],
        't850':  ['TEMPERATURE__ISOBARIC', 'T__ISOBARIC', 'TEMP__ISOBARIC'],
        't700':  ['TEMPERATURE__ISOBARIC', 'T__ISOBARIC', 'TEMP__ISOBARIC'],
        't500':  ['TEMPERATURE__ISOBARIC', 'T__ISOBARIC', 'TEMP__ISOBARIC'],
        'r850':  ['RELATIVE_HUMIDITY__ISOBARIC', 'RH__ISOBARIC', 'HUMIDITY'],
        'r700':  ['RELATIVE_HUMIDITY__ISOBARIC', 'RH__ISOBARIC', 'HUMIDITY'],
        'u_pl':  ['U_COMPONENT', 'UGRD', 'U-COMPONENT', 'WIND_U'],
        'v_pl':  ['V_COMPONENT', 'VGRD', 'V-COMPONENT', 'WIND_V'],
    }

    def find_cov(patterns):
        for pat in patterns:
            for cid in all_cov_ids:
                if pat.upper() in cid.upper():
                    return cid
        return None

    # Find coverage IDs dynamically
    CID_CAPE  = find_cov(WANT_MAP['cape'])
    CID_CIN   = find_cov(WANT_MAP['cin'])
    CID_T     = find_cov(WANT_MAP['t850'])
    CID_RH    = find_cov(WANT_MAP['r850'])
    CID_U     = find_cov(WANT_MAP['u_pl'])
    CID_V     = find_cov(WANT_MAP['v_pl'])

    log.info('Arpège coverage IDs found: CAPE=%s CIN=%s T=%s RH=%s U=%s V=%s',
             CID_CAPE, CID_CIN, CID_T, CID_RH, CID_U, CID_V)

    if not CID_T:
        log.warning('Arpège: could not find temperature coverage ID — API may have changed')
        log.warning('Available IDs: %s', all_cov_ids[:20])
        return None

    # Parameters: (coverage_id, output_key, pressure_levels_or_None, k_to_c)
    PARAMS = []
    if CID_CAPE: PARAMS.append((CID_CAPE, 'cape',  None,                False))
    if CID_CIN:  PARAMS.append((CID_CIN,  'cin',   None,                False))
    if CID_T:    PARAMS.append((CID_T,    't_pl',  [925,850,700,500],   True))
    if CID_RH:   PARAMS.append((CID_RH,   'r_pl',  [850,700,500],       False))
    if CID_U:    PARAMS.append((CID_U,    'u_pl',  [925,850,700,500,250], False))
    if CID_V:    PARAMS.append((CID_V,    'v_pl',  [925,850,700,500,250], False))
    ...
    for delta_h in range(0, 25, 6):
        dt   = now_utc - timedelta(hours=delta_h)
        gh   = (dt.hour // 6) * 6
        run_dt = dt.replace(hour=gh, minute=0, second=0, microsecond=0)
        break
    date_s = run_dt.strftime('%Y%m%d')
    run_s  = '%02d' % run_dt.hour
    ...
    idxs = far = None
    raw = {}
    arp_tmpdir = Path(tmpdir) / 'arpege'
    arp_tmpdir.mkdir(exist_ok=True)

    # ── 3. Fetch each parameter + step ───────────────────────────────────────
    for step in STEPS:
        valid_dt  = run_dt + timedelta(hours=step)
        valid_str = valid_dt.strftime('%Y-%m-%dT%H:%M:%SZ')

        for cov_id, key, levels, k2c in PARAMS:
            lvl_list = levels or [0]
            for lv in lvl_list:
                subsets = [
                    f'time({valid_str})',
                    f'lat({LAT_MIN-1},{LAT_MAX+1})',
                    f'long({LON_MIN-1},{LON_MAX+1})',
                ]
                if levels:
                    subsets.append(f'pressure({lv})')

                try:
                    r = requests.get(f'{BASE}/GetCoverage', params={
                        'SERVICE':    'WCS',
                        'VERSION':    '2.0.1',
                        'coverageId': cov_id,
                        'subset':     subsets,
                    ...
                    if not r.ok:
                        log.debug('Arpège %s lv=%s step=%d: HTTP %d',
                                  key, lv, step, r.status_code)
                        continue

                    # Write to temp file (cfgrib needs real path)
                    tf = arp_tmpdir / f'{key}_{lv}_step{step:03d}.grib2'
                    ...
                    for ds in dsets:
                        ...
                        if idxs is None and fl_la.size > 100:
                            _tree = cKDTree(np.column_stack([fl_la, fl_lo]))
                            dists, idxs = _tree.query(
                                np.column_stack([flat_la, flat_lo]), k=1)
                            far = dists > 0.5
                            log.info('Arpège KDTree built: %d pts', fl_la.size)
                        ...
                        for var in ds.data_vars:
                            ...
                            if flat_v.size != fl_la.size:
                                continue
                            out_key = f'{key}_{lv}' if levels else key
                            if k2c and flat_v.mean() > 100:
                                flat_v = flat_v - 273.15
                            v = flat_v[idxs].copy(); v[far] = 0.0
                            raw.setdefault(out_key, {})[step] = v

                    tf.unlink(missing_ok=True)

                except Exception as e:
                    log.debug('Arpège step %d %s lv=%s: %s', step, key, lv, e)

    # Clean up temp dir
    try:
        import shutil as _shutil
        _shutil.rmtree(str(arp_tmpdir), ignore_errors=True)
    except Exception:
        pass

    if idxs is None or not any(raw.get(k) for k in ['cape', 't_pl_850']):
        log.warning('Arpège: no usable data after fetching — check API key and coverage IDs')
        return None

    # ── 4. Build step dicts ───────────────────────────────────────────────────
    def get_a(key, step, default=0.0):
        d = raw.get(key, {})
        if not d:
            return np.full(n, default, dtype=np.float32)
        s = step if step in d else min(d, key=lambda x: abs(x-step))
        return d[s].copy() if abs(s-step) <= 9 else np.full(n, default, dtype=np.float32)
    ...
    data_out = []
    for step in STEPS:
        cape = np.maximum(get_a('cape', step), 0)
        cin  = get_a('cin',  step); cin = np.where(cin>0, -cin, cin)
        t925 = get_a('t_pl_925', step); t850 = get_a('t_pl_850', step)
        t700 = get_a('t_pl_700', step); t500 = get_a('t_pl_500', step)
        r850 = get_a('r_pl_850', step, 60.0); r700 = get_a('r_pl_700', step, 55.0)
        r925 = get_a('r_pl_925', step, 75.0) if 'r_pl_925' in raw else r850 + 10
        u925 = get_a('u_pl_925', step); v925 = get_a('v_pl_925', step)
        u850 = get_a('u_pl_850', step); v850 = get_a('v_pl_850', step)
        u700 = get_a('u_pl_700', step); v700 = get_a('v_pl_700', step)
        u250 = get_a('u_pl_250', step); v250 = get_a('v_pl_250', step)
    ...
    log.info('Arpège data built: %d steps, run %s/%sz', len(data_out), date_s, run_s)
    return data_out, (date_s, run_s)


# ═══════════════════════════════════════════════════════════════════════════════
# OPEN-METEO GRID FETCHER  (shared by HARMONIE + MeteoSwiss)
# Open-Meteo is free, no auth, returns JSON with all pressure-level vars.
# We sample at 1° resolution over Europe (~1200 pts), then interpolate
# to our output grid using KDTree — same pattern as all other models.
# ═══════════════════════════════════════════════════════════════════════════════

def _fetch_openmeteo_grid(model_name, ref_lats, ref_lons,
                           sample_step=1.0, max_workers=30,
                           forecast_hours=72, label='OpenMeteo'):
    """
    Fetch gridded forecast data from Open-Meteo for any supported model.
    
    Samples the EU domain at `sample_step` degree resolution, fires
    parallel requests, then interpolates to the output grid.
    
    Returns list of step dicts (same format as build_icon_grid), or None.
    """
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from scipy.spatial import cKDTree

    # Build sample grid over Europe at coarser resolution
    s_lats = np.arange(LAT_MIN, LAT_MAX + sample_step, sample_step)
    s_lons = np.arange(LON_MIN, LON_MAX + sample_step, sample_step)
    SLG, SOG = np.meshgrid(s_lats, s_lons, indexing='ij')
    src_la = SLG.ravel()
    src_lo = SOG.ravel()
    n_src  = len(src_la)

    # Pressure level variables we need
    PL_VARS = []
    ...
    # Hourly times we want
    want_hours = STEPS[:int(forecast_hours/3)+1]  # every 3h up to limit

    BASE_URL = 'https://api.open-meteo.com/v1/forecast'

    def fetch_point(lat, lon, pt_idx):
        """Fetch one lat/lon point — returns (pt_idx, {var: [val_per_step]})."""
        try:
            ...
            # Build {var: list_of_values_at_3h_steps}
            out = {}
            for var in ALL_VARS:
                vals = hourly.get(var)
                if vals is None:
                    out[var] = [0.0] * len(STEPS)
                    continue
                # Subsample to every 3h
                step_vals = []
                for step in STEPS:
                    # Find index closest to this step hour
                    target_unix = data.get('hourly', {})
                    # Simple: take index = step (hourly data, want 3-hourly)
                    idx = step if step < len(vals) else len(vals) - 1
                    v = vals[idx]
                    step_vals.append(float(v) if v is not None else 0.0)
                out[var] = step_vals
            return pt_idx, out
        except Exception as e:
            log.debug('%s point (%.2f,%.2f): %s', label, lat, lon, e)
            return pt_idx, None

    # Parallel fetch
    log.info('%s: fetching %d sample points (model=%s)...', label, n_src, model_name)
    point_data = [None] * n_src
    ok_count = 0

    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futs = {ex.submit(fetch_point, src_la[i], src_lo[i], i): i
                for i in range(n_src)}
        done = 0
        for fut in as_completed(futs):
            done += 1
            if done % 200 == 0:
                log.info('%s: %d/%d points fetched', label, done, n_src)
            try:
                idx, result = fut.result()
                if result:
                    point_data[idx] = result
                    ok_count += 1
            except Exception:
                pass

    log.info('%s: %d/%d points OK', label, ok_count, n_src)
    # Regional models (AROME, ICON-D2) only cover part of the EU domain —
    # allow as few as 5% of points to have data before aborting.
    min_frac = 0.05
    if ok_count < n_src * min_frac:
        log.warning('%s: fewer than %.0f%% of points returned data — aborting',
                    label, min_frac * 100)
        return None

    # Build KDTree from sample grid → interpolate to output grid
    flat_la = np.array(ref_lats)
    flat_lo = np.array(ref_lons)
    n = len(flat_la)
    ...
    def get_var(var, step_i, default=0.0):
        """Get interpolated field for variable at step index."""
        out = np.full(n, default, dtype=np.float32)
        for pt in range(n):
            src_pt = idxs[pt]
            pd = point_data[src_pt]
            if pd and var in pd and step_i < len(pd[var]):
                out[pt] = pd[var][step_i]
            if far[pt]:
                out[pt] = default
        return out

    # Vectorised version (much faster than point loop)
    def get_var_fast(var, step_i, default=0.0):
        src_vals = np.full(n_src, default, dtype=np.float32)
        for pi in range(n_src):
            pd = point_data[pi]
            if pd and var in pd and step_i < len(pd[var]):
                src_vals[pi] = pd[var][step_i]
        out = src_vals[idxs]
        out[far] = default
        return out
    ...
    data_out = []
    for si, step in enumerate(STEPS):
        cape  = np.maximum(get_var_fast('cape', si), 0)
        t925  = get_var_fast('temperature_925hPa', si)
        t850  = get_var_fast('temperature_850hPa', si)
        t700  = get_var_fast('temperature_700hPa', si)
        t500  = get_var_fast('temperature_500hPa', si)
        r925  = get_var_fast('relative_humidity_925hPa', si, 75.0)
        r850  = get_var_fast('relative_humidity_850hPa', si, 60.0)
        r700  = get_var_fast('relative_humidity_700hPa', si, 55.0)
        u925  = get_var_fast('wind_u_component_925hPa', si)
        v925  = get_var_fast('wind_v_component_925hPa', si)
        u850  = get_var_fast('wind_u_component_850hPa', si)
        v850  = get_var_fast('wind_v_component_850hPa', si)
        u700  = get_var_fast('wind_u_component_700hPa', si)
        v700  = get_var_fast('wind_v_component_700hPa', si)
        u500  = get_var_fast('wind_u_component_500hPa', si)
        v500  = get_var_fast('wind_v_component_500hPa', si)
        u250  = get_var_fast('wind_u_component_250hPa', si)
        v250  = get_var_fast('wind_v_component_250hPa', si)

        # Fill missing 925 from 850 estimate
        if not np.any(t925 != 0): t925 = t850 + 5.0
        if not np.any(t700 != 0): t700 = (t850 + t500) / 2.0
        ...
        sh1  = np.hypot(u850-u925, v850-v925); np.nan_to_num(sh1, copy=False)
        sh3  = np.hypot(u700-u925, v700-v925); np.nan_to_num(sh3, copy=False)
        cin  = np.zeros(n, dtype=np.float32)   # Open-Meteo CIN not in free tier
        ...
        data_out.append({
            'cape': col(cape), 'cin': col(cin), 'kx': col(kx),
            'li': col(li), 'tt': col(tt),
            't925': col(t925), 't850': col(t850),
            't700': col(t700), 't500': col(t500),
            'td925': col(td925), 'td850': col(td850),
            'r925': col(r925, 75.0), 'r850': col(r850, 60.0), 'r700': col(r700, 55.0),
            'sh6': col(sh6), 'sh1': col(sh1), 'sh3': col(sh3),
            'srh1': col(srh1), 'srh3': col(srh3),
...
def fetch_harmonie(ref_lats, ref_lons):
    """
    KNMI HARMONIE-AROME Europe via Open-Meteo (free, no auth).
    2km resolution, 60h forecast, updates every 3h.
    Uses 'knmi_harmonie_arome_europe' model on Open-Meteo.
    """
    log.info('HARMONIE: fetching via Open-Meteo (knmi_harmonie_arome_europe)...')
    result = _fetch_openmeteo_grid(
        model_name   = 'knmi_harmonie_arome_europe',
        ref_lats     = ref_lats,
        ref_lons     = ref_lons,
        sample_step  = 1.0,
        max_workers  = 40,
...
def fetch_meteoswiss(ref_lats, ref_lons):
    """
    MeteoSwiss ICON-CH2 via Open-Meteo (free, no auth).
    2km resolution, 120h forecast, updates every 6h.
    Covers Central Europe — scores blended with ICON-EU for full domain.
    """
    log.info('MeteoSwiss: fetching via Open-Meteo (meteoswiss_icon_ch2)...')
    result = _fetch_openmeteo_grid(
        model_name   = 'meteoswiss_icon_ch2',
        ref_lats     = ref_lats,
        ref_lons     = ref_lons,
        sample_step  = 0.5,   # smaller step — CH2 only covers Central EU
        max_workers  = 40,
        forecast_hours = 72,
        label        = 'MeteoSwiss',
    )
...
def fetch_cmc(ref_lats, ref_lons):
    """CMC GEM Global (Canada) via Open-Meteo — free, no auth, global coverage."""
    log.info('CMC GEM: fetching via Open-Meteo...')
    return _fetch_openmeteo_grid(
        model_name='gem_seamless', ref_lats=ref_lats, ref_lons=ref_lons,
        sample_step=1.0, max_workers=80, forecast_hours=72, label='CMC')


def fetch_arpege_openmeteo(ref_lats, ref_lons):
    """ARPEGE Europe via Open-Meteo — free, no auth, no expiring tokens."""
    log.info('ARPEGE: fetching via Open-Meteo...')
    return _fetch_openmeteo_grid(
        model_name='meteofrance_arpege_europe', ref_lats=ref_lats, ref_lons=ref_lons,
        sample_step=1.0, max_workers=80, forecast_hours=72, label='ARPEGE')


def fetch_arome(ref_lats, ref_lons):
    """Meteo-France AROME (W Europe, 1.3km) via Open-Meteo — free, no auth."""
    log.info('AROME: fetching via Open-Meteo...')
    return _fetch_openmeteo_grid(
        model_name='meteofrance_arome_france', ref_lats=ref_lats, ref_lons=ref_lons,
        sample_step=1.0, max_workers=80, forecast_hours=42, label='AROME')


def fetch_icon_d2(ref_lats, ref_lons):
    """DWD ICON-D2 (Central Europe, 2km) via Open-Meteo — free, no auth."""
    log.info('ICON-D2: fetching via Open-Meteo...')
    return _fetch_openmeteo_grid(
        model_name='icon_d2', ref_lats=ref_lats, ref_lons=ref_lons,
        sample_step=1.0, max_workers=80, forecast_hours=48, label='ICON-D2')
...
def load_thread():
    ...
    try:
        ...
        # ── ICON-EU: probed and detected above, runs every 3h ────────────────
        ...
        # ── GFS deterministic: 0.25°, 4x/day, NOAA public S3 ─────────────
        prog(52, 'Fetching NOAA GFS...')
        gfs_det_data = None
        try:
            result = fetch_gfs_det(tmpdir, flat_la.tolist(), flat_lo.tolist())
            if result:
                gfs_det_data, gfs_det_run = result
                log.info('GFS det data built: %d steps', len(gfs_det_data))
        except Exception as e:
            log.warning('GFS fetch failed (non-fatal): %s', e)

        # ── ECMWF IFS: 00z/12z runs, free open data via ecmwf-opendata ─────
        ...
        # ── Open-Meteo models — fetch ALL simultaneously in parallel threads ─
        # HARMONIE / CMC / ARPEGE / AROME / ICON-D2 all via Open-Meteo.
        # Each model's async HTTP runs inside its own thread so they don't
        # block each other — total time = slowest single model, not their sum.
        prog(60, 'Fetching Open-Meteo models in parallel (HARMONIE/CMC/ARPEGE/AROME/ICON-D2)...')

        from concurrent.futures import ThreadPoolExecutor as _TPE, as_completed as _ac

        _OM_MODELS = {
            'cmc':      fetch_cmc,
            'arpege':   fetch_arpege_openmeteo,
            'arome':    fetch_arome,
        }
        _ref_la = flat_la.tolist()
        _ref_lo = flat_lo.tolist()
        _om_results = {k: None for k in _OM_MODELS}

        with _TPE(max_workers=len(_OM_MODELS)) as _pool:
            _fut_map = {_pool.submit(fn, _ref_la, _ref_lo): name
                        for name, fn in _OM_MODELS.items()}
            for _fut in _ac(_fut_map):
                _name = _fut_map[_fut]
                try:
                    _om_results[_name] = _fut.result()
                    _r = _om_results[_name]
                    log.info('%s data built: %d steps', _name, len(_r) if _r else 0)
                except Exception as _e:
                    log.warning('%s fetch failed (non-fatal): %s', _name, _e)

        harmonie_data = None
        cmc_data      = _om_results['cmc']
        arpege_data   = _om_results['arpege']
        arome_data    = _om_results['arome']
        icon_d2_data  = None

        gefs_data = None  # GEFS removed — use ECMWF + deterministic models instead

        if not any([icon_data, ecmwf_data, cmc_data, gfs_det_data, harmonie_data, arpege_data, arome_data, icon_d2_data]):
            prog(0, 'No model data available — all sources failed')
            return

        # Build times list from ICON or GEFS step count
        n_steps = len(next(d for d in [icon_data, ecmwf_data, cmc_data, gfs_det_data, harmonie_data, arpege_data, arome_data, icon_d2_data] if d))
        ...
        if gfs_det_data:
            ...
            models_raw['harmonie'] = harmonie_data
        if arpege_data:
            models_raw['arpege']   = arpege_data
        if arome_data:
            models_raw['arome']    = arome_data
            models_raw['icon_d2']  = icon_d2_data

        # Score all available models — skip any that returned None
        prog(75, 'Computing hazard scores...')
        models_raw = {k: v for k, v in models_raw.items() if v is not None}
        models_scores = {}
        import base64
...
# ═══════════════════════════════════════════════════════════════════════════════
# AR-CHaMo: Additive Regression Convective Hazard Model
# Two-stage logistic probability system (Rädler et al. 2018/2023, ESSL/NHESS)
#
# Stage 1 — P(storm):  probability of thunderstorm occurrence
#   Predictors: log(CAPE), SH6, K-Index, LI, RH850, CIN
#   Calibrated against EUCLID lightning network 2006-2019 over Europe
#
# Stage 2 — P(hazard|storm): conditional probability of each hazard
#   Hail ≥2cm:    CAPE, SH6, mixing ratio, wet-bulb zero height
#   Wind ≥25m/s:  CAPE, DCAPE, SH6, 850-500 lapse rate
#   Tornado:      STP, SRH 0-3km, LCL height, 0-1km shear
#
# Final: P(severe) = P(storm) × max(P(hail), P(wind), P(tornado))
#
# Output: SPC risk level 0-6 (float, for smooth Gaussian blending)
...
def _archamo_p_storm(cape, sh6, kx, li, r850, cin):
    """
    AR-CHaMo Stage 1: P(thunderstorm).
    Recalibrated for European climatology — TSTM requires genuinely convective
    environments (CAPE ≥ ~300 J/kg with moisture + some shear).
    """
    log_cape = np.log1p(np.maximum(cape, 0.0))  # 0 → 8.5 for 0→5000 J/kg
    sh6_n    = np.clip(sh6,  0.0,  40.0)
    kx_n     = np.clip(kx,  -20.0, 50.0)
    li_n     = np.clip(li,  -10.0,  5.0)
    rh_n     = np.clip(r850, 20.0, 100.0)
    cin_n    = np.clip(cin, -300.0,  0.0)

    # Intercept tightened to -6.5 so that P(storm)=10% requires
    # CAPE ~500 J/kg + KI ~25 + adequate moisture simultaneously.
    # log(500)≈6.2 → 0.45*6.2=2.8; kx=25 → 0.09*25=2.25 → sum ≈ 5.05
    # Still need RH boost to cross threshold → realistic for Europe.
    logit = (-6.50
             + 0.45 * log_cape
             + 0.05 * sh6_n
             + 0.09 * kx_n
...
def _archamo_p_hail(cape, sh6, r850, t850, t500):
    """
    AR-CHaMo Stage 2: P(hail ≥ 2cm | storm).
    Requires genuine supercell/organised convection environment:
    CAPE ≥ 1000 J/kg + SH6 ≥ 15 m/s + sufficient moisture.
    """
    log_cape = np.log1p(np.maximum(cape, 0.0))
    sh6_n    = np.clip(sh6, 0.0, 40.0)

    es   = 6.112 * np.exp(17.67 * t850 / (t850 + 243.5))
    rh_c = np.clip(r850, 1.0, 100.0)
    e    = es * rh_c / 100.0
    mixr = np.clip(1000.0 * 0.622 * e / (850.0 - e), 0.0, 25.0)

    t850_safe = np.where(t850 > t500, t850, t500 + 0.1)
    wbz_frac  = np.clip(t850_safe / (t850_safe - t500), 0.0, 1.0)
    wbz_hgt   = 1500.0 + wbz_frac * 4000.0

    lr75 = np.clip((t850 - t500) / 3.5, 0.0, 12.0)

    # Intercept tightened: needs CAPE~2000 + SH6~20 + good mixr to reach 5%
    logit = (-8.50
             + 0.42 * log_cape
             + 0.14 * sh6_n
             + 0.16 * mixr
...
def _archamo_p_wind(cape, dcape, sh6, t850, t500):
    """
    AR-CHaMo Stage 2: P(severe wind ≥ 25 m/s | storm).
    Needs DCAPE ≥ 500 J/kg + steep lapse rates to reach meaningful probability.
    """
    log_cape = np.log1p(np.maximum(cape,  0.0))
    dcape_n  = np.clip(dcape, 0.0, 2000.0)
    sh6_n    = np.clip(sh6,   0.0,  40.0)
    lr_km    = np.clip((t850 - t500) / 3.5, 0.0, 12.0)

    # Intercept tightened: DCAPE ~800 + SH6 ~20 + LR ~7.5 needed for 5%
    logit = (-8.00
             + 0.28 * log_cape
             + 0.0028 * dcape_n
             + 0.12 * sh6_n
...
def _archamo_p_tornado(stp, srh3, sh1, lcl_h):
    """
    AR-CHaMo Stage 2: P(tornado | storm).
    STP ≥ 1 + SRH3 ≥ 150 m²/s² required for meaningful probability.
    """
    stp_n  = np.clip(stp,   0.0,  15.0)
    srh3_n = np.clip(srh3,  0.0, 700.0)
    sh1_n  = np.clip(sh1,   0.0,  25.0)
    lcl_n  = np.clip(lcl_h, 0.0, 3000.0)

    # Intercept tightened: STP~2 + SRH3~250 needed to reach 5%
    logit = (-9.00
             + 0.70 * stp_n
             + 0.0050 * srh3_n
             + 0.18 * sh1_n
...
def _archamo_spc_level(cape, cin, sh6, kx, li, r850, t850, t500,
                        dcape, stp, srh3, sh1, lcl_h):
    """
    Full AR-CHaMo pipeline: returns SPC risk level (0.0–6.0) per grid point.

    P(storm)  = logistic(CAPE, SH6, KI, LI, RH850, CIN)
    P(hail)   = logistic(CAPE, SH6, mixr, WBZ) × P(storm)
    P(wind)   = logistic(CAPE, DCAPE, SH6, LR)  × P(storm)
    P(tornado)= logistic(STP, SRH3, SH1, LCL)   × P(storm)
    P(severe) = max(P(hail), P(wind), P(tornado))

    SPC level thresholds:
      TSTM  (1): P(storm)  ≥ 10%
      MRGL  (2): P(severe) ≥  5%
      SLGT  (3): P(severe) ≥ 15%
      ENH   (4): P(severe) ≥ 30%
      ...
      HIGH  (6): P(severe) ≥ 60%
    """
    p_storm = _archamo_p_storm(cape, sh6, kx, li, r850, cin)
    p_hail  = _archamo_p_hail(cape, sh6, r850, t850, t500) * p_storm
    p_wind  = _archamo_p_wind(cape, dcape, sh6, t850, t500) * p_storm
    p_tor   = _archamo_p_tornado(stp, srh3, sh1, lcl_h)     * p_storm
    p_sev   = np.maximum.reduce([p_hail, p_wind, p_tor])

    # Assign SPC level as a float so Gaussian blending gives smooth gradients
    level = np.zeros(len(cape), dtype=np.float32)
    level = np.where(p_storm >= 0.10, 1.0, level)   # TSTM
    level = np.where(p_sev   >= 0.05, 2.0, level)   # MRGL
    level = np.where(p_sev   >= 0.15, 3.0, level)   # SLGT
    level = np.where(p_sev   >= 0.30, 4.0, level)   # ENH
    level = np.where(p_sev   >= 0.45, 5.0, level)   # MDT
    level = np.where(p_sev   >= 0.60, 6.0, level)   # HIGH

    return level


def _archamo_score(cape, cin, kx, t850, t500, r850, sh6, srh1, stp, ehi,
                   tt=None, srh3=None, li=None,
                   dcape=None, sh1=None, lcl_h=None):
    """
    Unified AR-CHaMo scorer — called by SCORE_FNS for all hazards.
    Returns SPC level (0-6) for the combined convective outlook.
    """
    if srh3  is None: srh3  = srh1 * 1.6
    if li    is None: li    = np.zeros_like(cape)
    if dcape is None: dcape = np.zeros_like(cape)
    if sh1   is None: sh1   = np.zeros_like(cape)
    if lcl_h is None: lcl_h = np.full_like(cape, 1500.0)

    return _archamo_spc_level(
        cape=cape, cin=cin, sh6=sh6, kx=kx, li=li,
        r850=r850, t850=t850, t500=t500,
        dcape=dcape, stp=stp, srh3=srh3, sh1=sh1, lcl_h=lcl_h,
    )


# All hazards use the same combined AR-CHaMo SPC level.
# The SCORE_FNS signature is kept identical to the old system for compatibility.
SCORE_FNS = {
    'tstm':    _archamo_score,
    'hail':    _archamo_score,
    'wind':    _archamo_score,
    'tornado': _archamo_score,
}
