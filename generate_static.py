...
def run_pipeline(icon_only=False, gefs_only=False, no_gefs=False):
    """
    Main pipeline:
    ...
    # Define CalibratedModel in __main__ scope BEFORE importing server so that
    # pickle can find it when deserialising convective_models.pkl
    # (pickle looks up class by module path — __main__ is generate_static.py)
    import lightgbm as _lgb  # must be imported before pickle.load
    import sys as _sys
    ...
    class CalibratedModel:
        ...
        def predict_proba(self, X):
            ...
            return _np.column_stack([1 - _np.clip(cal,0,1), _np.clip(cal,0,1)])

    # Inject into __main__ so pickle finds it
    _sys.modules[__name__].CalibratedModel = CalibratedModel
    ...
    if icon_only or no_gefs:
        log.info('%s: GEFS will be skipped', 'icon-only' if icon_only else 'no-gefs')
    ...
    if icon_only:
        # Also skip ECMWF (icon-only = fastest, just ICON)
        log.info('icon-only mode: ECMWF will also be skipped')
        from ecmwf.opendata import Client as _OrigClient

        class _NoopClient:
            def retrieve(self, *a, **kw): pass
            def latest(self, *a, **kw): return None

        import ecmwf.opendata as _ecmwf_mod
        _ecmwf_mod.Client = _NoopClient
...
def main():
    ...
    parser.add_argument('--no-gefs', action='store_true',
                        help='Skip GEFS (fetch ICON-EU + ECMWF; faster)')
    ...
    # --no-gefs: skip GEFS but still run ECMWF + ICON
    # --icon-only: skip both GEFS and ECMWF (fastest)
    no_gefs = args.no_gefs or args.icon_only
